...
ANG::ANG(vector<int> hidden_layers, int num_outputs, int num_inputs) {
	vector<int> layers;
...
vector<int> ANG::sorted_index(int size, vector<float> b) {
...
void ANG::Print3D(vector<vector<vector<float>>> a) {
...
float ANG::stdev(vector<float> data) {
	int size = data.size();
...
void ANG::PrintMatrix(vector<vector<float>> a) {
	for (int i = 0; i < a.size(); i++) {
...
void ANG::PrintVector(vector<float> a) {
	for (int i = 0; i < a.size(); i++) cout << a[i] << " ";
}

vector<vector<float>> ANG::transpose(vector<vector<float>> b)
{
	if (b.size() == 0)
		return { {} };
...
void ANG::Init(vector<int> hidden_layers, int num_outputs, int num_inputs)
{
	vector<int> layers;
	layers.push_back(num_inputs);
	for (int i = 0; i < hidden_layers.size(); i++) layers.push_back(hidden_layers[i]);
...
vector<vector<float>> ANG::dot_new(vector<vector<float>>& a, vector<vector<float>>& b) {
...
//Dot product done
vector<vector<float>> ANG::dot_product(vector<vector<float>>& a, vector<vector<float>>& b) {
	int row_a, row_b, col_a, col_b;
	row_a = a.size();
	row_b = b.size();
	col_a = a[0].size();
...
float ANG::_mse(vector<float> target, vector<float> output) {
	float sum = 0;
	int n = target.size();
...
//sigmoid done
vector<float> ANG::_sigmoid(vector<float>& a) {
...
vector<float> ANG::_sigmoid_derivative(vector<float>& a) {
	vector<float> sigmoid;
	for (int i = 0; i < a.size(); i++) sigmoid.push_back(a[i] * (1 - a[i]));
	return sigmoid;
}
//forward propogation done
vector<float> ANG::forward_propogate(vector<float> inputs) {
...
vector<float> ANG::forward_propogate_test(vector<float> inputs) {
	activations[0] = inputs;
	vector<vector<float>> net_inputs;
	vector<vector<float>> activ;
	activ.push_back(inputs);
...
void ANG::gradient_descent(float learning_rate) {
	for (int i = 0; i < weights.size(); i++) {
...
void ANG::back_propogate(vector<float> error) {
...
void ANG::train(vector<vector<float>> inputs, vector<vector<float>> targets, int epochs, float learning_rate) {
...
void ANG::prime_base_network(vector<vector<float>> inputs, vector<vector<float>> targets, int cycles, float learning_rate) {
	train(inputs, targets, cycles, learning_rate);
	Print3D(weights);
}

void ANG::remove_temp_classifier() {
	weights.resize(weights.size() - 1);
	derivatives.resize(derivatives.size() - 1);
	activations.resize(activations.size() - 1);
}

void ANG::add_destination_layer() {
...
//done
void ANG::add_class_layer(vector<vector<float>> targets) {
	vector<float> temp;
...
// not done
void ANG::add_class_layer_final() {}

void ANG::create_temp_classifier_seed(vector<vector<float>> targets) {
	vector<float> temp;
...
vector<float> ANG::forward_propogate_seed(vector<float> inputs) {
...
void ANG::back_propogate_seed(vector<float> error) {
	vector<float> activation;
	//vector<float> delta;
	vector<float> sigmoid_derivative;
	//cout << activations.size();
...
void ANG::train_seed(vector<vector<float>> inputs, vector<vector<float>> targets, int epochs, float learning_rate) {
	for (int i = 0; i < epochs; i++) {
		float sum_errors = 0;
		for (int j = 0; j < inputs.size(); j++) {
			vector<float> target = targets[j];
...
void ANG::prime_seed_network(vector<vector<float>> inputs, vector<vector<float>> targets, int cycles, float learning_rate) {
	train_seed(inputs, targets, cycles, learning_rate);
}

void ANG::remove_temp_classifier_seed() {
...
vector<vector<vector<float>>> ANG::extreme_member_classes(vector<vector<float>> inputs, vector<vector<float>> targets) {
...
vector<float> ANG::return_source_activations() {
	return activations.rbegin()[1];
}

float ANG::return_acc(vector<vector<float>> inputs, vector<vector<float>> targets) {
...
void ANG::ANG_grow(vector<vector<float>> inputs, vector<vector<float>> targets) {
...
*/
void ANG::save_weights_der(vector<vector<vector<float>>> Matrix, string name) {
	//cout << "Matrix" << endl;
	//Print3D(Matrix);
...
void ANG::save_activations(vector<vector<float>> Matrix, string name) {
...
void ANG::read_weights_der(vector<vector<vector<float>>>& Matrix, string name) {
...
void ANG::read_activations(vector<vector<float>>& Act, string name) {
...
void ANG::save_model(string name) {
	//Print3D(weights);
...
void ANG::load_model(string name) {
